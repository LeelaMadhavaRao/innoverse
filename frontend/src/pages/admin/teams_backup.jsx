...
function AdminTeams() {
  ...
  const [newTeam, setNewTeam] = useState({
    ...
    teamSize: 3,
    teamMembers: ['', ''], // Default for team size 3 (leader + 2 members)
    projectDetails: {
      title: '',
      description: '',
      category: ''
  ...
  const handleTeamSizeChange = (size) => {
    ...
    const membersCount = newSize - 1; // Exclude leader
    
    setNewTeam(prev => ({
      ...prev,
      teamSize: newSize,
      teamMembers: Array(membersCount).fill('').map((_, index) => 
        prev.teamMembers[index] || ''
      )
    }));
  };
  ...
  const handleCreateTeam = async (e) => {
    ...
    if (!newTeam.teamLeader.name.trim() || !newTeam.teamLeader.email.trim()) {
      toast({
        title: 'Validation Error',
    ...
    const emptyMembers = newTeam.teamMembers.filter(member => !member.trim());
    ...
    try {
      ...
      const teamData = {
        ...
        teamLeader: {
          name: newTeam.teamLeader.name.trim(),
          email: newTeam.teamLeader.email.trim().toLowerCase(),
          phone: newTeam.teamLeader.phone.trim()
        },
        teamSize: newTeam.teamSize,
        teamMembers: newTeam.teamMembers.filter(member => member.trim()),
        projectDetails: {
          title: newTeam.projectDetails.title.trim(),
          description: newTeam.projectDetails.description.trim(),
          category: newTeam.projectDetails.category.trim()
        },
        customPassword: newTeam.customPassword.trim() || undefined
      };
                  ...
                  <form onSubmit={handleCreateTeam} className="space-y-6">
                    ...
                    <div className="flex gap-4 pt-4 border-t border-gray-600">
                      <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                        <Button
                          type="submit"
                          disabled={loading || !newTeam.teamSize}
                        ...
                        >
                          {loading ? '⏳ Creating Team...' : '✅ Create Team & Send Credentials'}
                        </Button>
                      </motion.div>
                      <Button
                        type="button"
                      ...
                      >
                        <div className="flex items-start justify-between mb-4">
                          <div>
                            ...
                            <p className="text-blue-400">Led by {team.leaderName}</p>
                          </div>
                        ...
                        <div className="space-y-3 mb-4">
                          <div>
                            ...
                            <span className="text-gray-300 text-sm">{team.leaderEmail}</span>
                          </div>
                          <div>
                            <span className="text-gray-400 text-sm">Members: </span>
                            <span className="text-gray-300 text-sm">{team.members.length}</span>
                          </div>
                          <div>
                        ...
                        <div className="flex gap-2">
                          <Button
                            size="sm"
                            onClick={() => resendCredentials(team)}
